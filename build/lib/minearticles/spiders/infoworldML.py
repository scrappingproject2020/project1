...
class InfoworldmlSpider(scrapy.Spider):
    name = 'infoworldML'
    allowed_domains = ['www.infoworld.com']
    start_urls = ['https://www.infoworld.com/category/machine-learning']

    def parse(self, response):
        articles = response.xpath("//div[@class='post-cont']")
        for article in articles[3:]:
            title = article.xpath(".//h3/a/text()").get()
            link = article.xpath(".//h3/a/@href").get()
            blurp = article.xpath(".//h4/text()").get()
            article_url = f"https://www.infoworld.com{link}"
        ...
        if next_page and next_page != '?start=80':
            full_url = f"https://www.infoworld.com/category/machine-learning/{next_page}"
    ...
    def parse_article(self,response):
        ...
        paragraphs = response.xpath("//section[@class='bodee']/div/p")
        text =''
